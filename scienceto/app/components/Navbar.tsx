...
export default function Navbar() {
    ...
    return (
        <nav className="fixed top-0 w-full z-50 border-b border-white/10 bg-black/60 backdrop-blur-md">
            <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden md:flex gap-8 items-center mr-4">
                        {navLinks.map((link) => (
                            <Link key={link.name} href={link.href} className="text-sm font-medium text-zinc-400 hover:text-white transition">
                                {link.name}
                            </Link>
                        ))}
                    </div>
                    ...
                    <div className="flex items-center gap-4 border-l border-white/10 pl-5">
                        {/* GitHub Icon */}
                        <Link
                            href="https://github.com/pateljheel"
                            target="_blank"
            ...
            {isOpen && (
                <div className="md:hidden bg-black border-b border-white/10 px-4 py-6 flex flex-col gap-5 animate-in slide-in-from-top">
                    {navLinks.map((link) => (
                        <Link
                            key={link.name}
                            href={link.href}
